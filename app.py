...
Enhanced Multi-Source CCTV System Backend with Ultra Low-Latency Streaming
Universal .pt Model Detection System with Smart Priority
Enhanced RTSP URL Support with All Standard Formats
Supports: RTSP/ONVIF, Webcams, Live Streams, Files - All with YOLO AI Detection
Enhanced with Live Stream Original Timing Preservation & Fixed File Timing
Added Toggleable Detection Overlay Feature
...
class MultiSourceCCTV:
    ...
    def parse_rtsp_url(self, rtsp_url, fallback_username="", fallback_password=""):
        ...
        try:
            ...
            if match:
                ...
                logger.info(f"🔍 RTSP URL parsed successfully:")
                ...
                logger.warning(f"⚠️ Invalid RTSP URL format: {rtsp_url}")
                return {
                    'success': False,
            ...
            logger.error(f"❌ RTSP URL parsing error: {e}")
            return {
                'success': False,
                'error': str(e),
                'clean_url': rtsp_url,  # Fallback ke URL asli
    ...
    def connect_rtsp(self, config):
        ...
        logger.info(f"📹 Connecting to RTSP with enhanced URL parsing...")
        ...
        if parsed['success']:
            ...
            if provided_camera_ip:
                ...
                logger.info(f"🎯 Using provided camera IP: {provided_camera_ip}")
            elif parsed.get('camera_ip'):
                self.camera_ip = parsed['camera_ip']
                logger.info(f"🎯 Using auto-extracted camera IP: {parsed['camera_ip']}")
            else:
                self.camera_ip = ""
                logger.info(f"🎯 No camera IP for ONVIF")
            
            # Log final configuration
            if parsed.get('has_credentials_in_url'):
                logger.info(f"✅ Using credentials from URL")
            elif self.username:
                logger.info(f"✅ Using fallback credentials")
            else:
                logger.info(f"🔓 No authentication")
                
        else:
            # Fallback jika parsing gagal
            logger.warning(f"⚠️ RTSP parsing failed: {parsed.get('error', 'Unknown error')}")
        ...
        # ULTRA LOW LATENCY CONNECTION SETTINGS
        logger.info(f"🚀 Establishing RTSP connection...")
        ...
        if not self.cap.isOpened():
            ...
            if parsed.get('has_credentials_in_url'):
                error_msg += "\n💡 Tip: Check if URL credentials are correct"
            elif not self.username:
                error_msg += "\n💡 Tip: Camera might require authentication"
        ...
        if not ret or frame is None:
            ...
            if parsed.get('has_credentials_in_url'):
                error_msg += "\n💡 Tip: URL credentials might be invalid"
            return False, error_msg
...
if __name__ == '__main__':
    ...
    print("   🎥 RTSP/IP Cameras with Enhanced URL Support ✅")
    ...
    print("\n🎯 ENHANCED FEATURES:")
    print("   • Enhanced RTSP URL parsing (all standard formats) ✅")
    print("   • Auto-credential extraction from RTSP URLs ✅")
    print("   • Smart IP detection for ONVIF PTZ control ✅")
    print("   • RTSP URL templates (Hikvision, Dahua, Axis) ✅")
    ...
    print("   • RTSP connection error notifications ✅")
    ...
    print("   • RTSP URL templates with error notifications ✅")
    print("   • Real-time performance monitoring ✅")
    ...
    print("   • Enhanced RTSP error handling ✅")
    
    print("\n💡 QUICK START EXAMPLES:")
    print("   📹 RTSP Basic: rtsp://192.168.1.100:554/stream1")
    print("   🔐 RTSP Auth: rtsp://admin:admin@192.168.1.100:554/stream1")
    print("   👤 RTSP User: rtsp://admin:@192.168.1.100:554/stream1")
    print("   📹 Hikvision: rtsp://admin:admin@IP:554/Streaming/Channels/101")
    print("   📹 Dahua: rtsp://admin:admin@IP:554/cam/realmonitor?channel=1&subtype=0")
    ...
    try:
        ...
        print("📹 Enhanced RTSP URL Support: ENABLED")
        print("   • Supports all standard RTSP URL formats ✅")
        print("   • Auto-credential extraction from URLs ✅")
        print("   • Smart IP detection for ONVIF PTZ ✅")
        print("   • URL format validation and parsing ✅")
        print("   • Brand-specific templates (Hikvision, Dahua, Axis) ✅")
        print("   • Real-time RTSP error notifications ✅")
